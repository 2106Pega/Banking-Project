package com.company.Controllers.Endpoints.WithdrawlEndpoint;
...
public class WithdrawlEnpoint implements Endpoint {
    @Override
    public String[] response(String[] parameters) {
        try{
            ...
            double withdrawlAmount = Double.parseDouble(formatter.format(Double.parseDouble(parameters[0])));
            if ( withdrawlAmount > this.account.getBalance()){
                return new String[]{
                        "error",
                        "Withdrawl must be less than balance"
                };
            }else{
                this.account.setBalance(this.account.getBalance()-withdrawlAmount);
                new AccountDAOImpl().saveAccount(this.account);
                new TransactionDAOImpl().addTransaction("withdrawl",this.account.getAccount_id(), withdrawlAmount);
                return new String[]{
                        "success",
                        "my_accounts_view"
                };
